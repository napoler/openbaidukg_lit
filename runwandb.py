...
class LitAutoLM(pl.LightningModule):
    ...
    def __init__(self, learning_rate=3e-4, T_max=500, optimizer_name="AdamW", **kwargs):
        super().__init__()
        self.hparams.config = BertConfig.from_pretrained(
...
#         configuration = BertConfig.from_pretrained("uer/roberta-tiny-word-chinese-cluecorpussmall")
        ...
        self.model = BertForMaskedLM.from_pretrained(
            "uer/roberta-small-word-chinese-cluecorpussmall", config=self.hparams.config)
...
#         logits = outputs.logits
    ...
    def pred(self, text, ner1, ner2):
        # training_step defined the train loop.
        # It is independent of forward
        inputs = self.tokenizer([ner1+"和"+ner2+"关系为"+"".join(["[MASK]"]*10)], [text],
...
#         return kgtext[i+3],kgtext
    ...
    def pred2(self, text, ner1, p):
        # training_step defined the train loop.
        # It is independent of forward
        inputs = self.tokenizer([ner1+p+"为"+"".join(["[MASK]"]*10)], [text],
                                return_tensors="pt", padding="max_length", max_length=128, truncation=True)
        outputs = self(**inputs)
        logits = outputs.logits
        kgtext = []
...
#             optimizer = self.get_optimizer()
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=self.hparams.T_max, T_mult=T_mult, eta_min=0, verbose=False)
...
sweep_config = {
    # Random search https://docs.wandb.ai/guides/sweeps/configuration#search-strategy
    "method": "bayes",
...
sweep_id = wandb.sweep(
    sweep_config, project="optuna优化_百度数据做开放关系BD_KnowledgeExtractionnoteb")
...
def sweep_iteration():
    ...
    train_loader = DataLoader(
        traindataset, batch_size=wandb.config.batch_size, shuffle=True, )
    val_loader = DataLoader(
        devdataset, batch_size=wandb.config.batch_size, shuffle=False, )
#     test_loader=DataLoader(testdataset,batch_size=wandb.config.batch_size, shuffle=False, )
    model = LitAutoLM(learning_rate=wandb.config.learning_rate,
